package com.lanchonete.order.adapter.driver.rest.controllers.mvc;
...
import com.lanchonete.order.adapter.driver.rest.controllers.ProductController;
...
class ProductControllerMvcTest {
    ...
    void shouldFindProductsByCategorySuccessfully() throws Exception {
        List<Product> products = Arrays.asList(product);
        when(findProductUseCase.findProductsByCategory(ProductCategoryEnum.LANCHE)).thenReturn(products);

        mockMvc.perform(get("/products/category/LANCHE"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].itemId").value(product.getId()))
                .andExpect(jsonPath("$[0].category").value("LANCHE"));

        verify(findProductUseCase, times(1)).findProductsByCategory(ProductCategoryEnum.LANCHE);
    }

    @Test
    void shouldReturnEmptyListWhenNoProductsFoundByCategory() throws Exception {
        when(findProductUseCase.findProductsByCategory(ProductCategoryEnum.BEBIDA)).thenReturn(Arrays.asList());

        mockMvc.perform(get("/products/category/BEBIDA"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray())
                .andExpect(jsonPath("$").isEmpty());

        verify(findProductUseCase, times(1)).findProductsByCategory(ProductCategoryEnum.BEBIDA);
    }
    ...
    void shouldReturn400WhenCreatingProductWithInvalidData() throws Exception {
        ...
        verify(createProductUseCase, never()).create(any(Product.class));
    }

    @Test
    void shouldReturn400WhenUpdatingProductWithInvalidData() throws Exception {
        ProductRequest invalidRequest = new ProductRequest();
        invalidRequest.setItemId(""); // Invalid: empty itemId
        invalidRequest.setName(""); // Invalid: empty name
        invalidRequest.setPrice(new BigDecimal("-10.00")); // Invalid: negative price
        ...
        mockMvc.perform(put("/products/PROD-001")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(invalidRequest)))
                .andExpect(status().isBadRequest());

        verify(updateProductUseCase, never()).update(anyString(), any(Product.class));
    }

    @Test
    void shouldReturn404WhenUpdatingNonExistentProduct() throws Exception {
        String productId = "PROD-999";
        when(updateProductUseCase.update(anyString(), any(Product.class)))
                .thenThrow(new ProductNotFoundException("Product not found"));

        mockMvc.perform(put("/products/{itemId}", productId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(productRequest)))
                .andExpect(status().isNotFound());
    ...
    void shouldReturn404WhenDeletingNonExistentProduct() throws Exception {
        ...
        doThrow(new ProductNotFoundException("Product not found"))
                .when(deleteProductUseCase).delete(productId);

        mockMvc.perform(delete("/products/{itemId}", productId))
                .andExpect(status().isNotFound());

        verify(deleteProductUseCase, times(1)).delete(productId);
    }
}
